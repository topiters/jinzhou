...
//二维码名片结束
    ...
    public function loaddata() {
       ...
       // $this->assign('count',$count);
        ...
        foreach ($list as $k => $val) {
        	//商家订单总数
        	$condition=array('shop_id'=>$list[$k]['shop_id']);
        	$num = D('Ordergoods')->where($condition)->count();
        	$list[$k]['num']=$num;
        	
        	//商家宝贝总数
        	$condition=array('shop_id'=>$list[$k]['shop_id']);
        	$con = D('Goods')->where($condition)->count();
        	$list[$k]['con']=$con;
        	
        	
        	//微店
        	$Weidian = D('Weidian_details');
        	$weidianid=$Weidian->where('shop_id='.$list[$k]['shop_id'].' ')->find();
        	//$this->assign('weidian_id', $weidianid['id']);
        	$list[$k]['weidianid']=$weidianid['id'];
