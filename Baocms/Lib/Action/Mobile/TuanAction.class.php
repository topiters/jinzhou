...
class TuanAction extends CommonAction
{
    ...
    public function fenlei($cate_id){
    	...
    	$list = D('Tuancate')->order($orderby)->where($m)->limit(0, 1)->select();
    	
    	$this->assign('mingcheng', $list);
    	 
    	$map=array('parent_id'=>$cate_id);
    	$l = D('Tuancate')->order($orderby)->where($map)->select();
    
    	//获得所有子分类的cateid
    	if ($cate_id) {
    		$catids = D('Tuancate')->getChildren($cate_id);
    		if (!empty($catids)) {
    	...
    	$condition = array('audit' => 1, 'closed' => 0, 'city_id' => $this->city_id);
    	$tuans = $Goods->order(" (ABS(lng - '{$lng}') +  ABS(lat - '{$lat}') ) asc ")->where($condition)->select();
    	foreach ($tuans as $k => $val) {
    		$tuans[$k]['d'] = getDistance($lat, $lng, $val['lat'], $val['lng']);
    	}
    	$this->assign('goods', $tuans);
    
    	//精选
    	$condition2=$condition;
    	$condition2['is_chose']=1;
    	$jingxuan = $Goods->order(" tuan_id desc ")->where($condition2)->select();
    	$this->assign("jingxuan",$jingxuan);
    	// $this->assign('city',$this->city_id);
    	//$this->assign('jingdu',$lat);
    	//$this->assign('weidu',$lng);
