...
//        dump($list);die;
    	...
    	$jp=D("Life")->where($map)->order("views desc")->limit(0,3)->select();
    	...
    	$near=D("Life")->where($map)->order(" (ABS(lng - '{$lng}') +  ABS(lat - '{$lat}') ) asc ")->select();
    	foreach ($near as $k => $v){
    		$near[$k]['d']=getDistance($lat, $lng, $v['lat'], $v['lng']);
    	}
    ...
    public function message() {
    	...
    	$life = $Life->where($map)->order(array('top_date' => 'desc', 'last_time' => 'desc'))->limit(0,4)->select();
    ...
    public function fabu($cat) {
        ...
        if ($this->isPost()) {
            ...
            if ($life_id = D('Life')->add($data)) {
                ...
                $this->fengmiMsg('发布信息成功，通过审核后将会显示！', U('life/fabu2'));
            }
   ...
   //发布成功后
	public function fabu2(){
		 ...
		 if ($this->isPost()) {
			...
			//var_dump($_POST);
			$this->success('发布信息成功，通过审核后将会显示！', U('life/index'));
