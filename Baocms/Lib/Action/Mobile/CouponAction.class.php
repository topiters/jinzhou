...
class CouponAction extends CommonAction {
    ...
    public function main() {
        ...
        //积分兑换
        $item=array("closed"=>0,"audit"=>1);
        $jifen=D("Integralgoods")->where($item)->order("orderby asc")->select();
...
//         }
        ...
        if ($detail['expire_date'] < TODAY) {
         ...
         if($detail['num'] <=0){
            $this->error('该代金券已经被领取完了');
        }
        
        if($detail['limit_num']){
            $count = D('Coupondownload')->where(array( 'coupon_id' => $coupon_id,'user_id'=>  $this->uid))->count();
            if($count+1 > $detail['limit_num']){
                $this->error('您已经超过领取该代金券的限制了！');
            }
        }
        
        $c=array("coupon_id"=>$coupon_id);
        $coupon=D("coupon")->where($c)->limit(0,1)->select();
        
        foreach ($coupon as $k=>$val){
        	
        	$condition=$val['condition'];
        	$time=$val['expire_date'];
        	$money=$val['money'];
        }
        ...
        if (D('Coupondownload')->add($data)) {
            ...
            $this->success('恭喜您领取成功！', U('mcenter/coupon/index'));
        }
        $this->error('领取失败！');
    }
