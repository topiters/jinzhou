...
//        dump($goodscates);die;
    ...
    public function fenlei($cate_id){
    	$Goods = D('Goods');
    	//import('ORG.Util.Page');
    	//获得一级分类下的二级分类
    	$cate_id = (int) $cate_id;
    	$m=array('cate_id'=>$cate_id);
    	$list = D('Goodscate')->order($orderby)->where($m)->limit(0, 1)->select();
    	$this->assign('mingcheng', $list);
    	$map=array('parent_id'=>$cate_id);
    	$l = D('Goodscate')->order($orderby)->where($map)->select();
    	
    	//获得所有子分类的cateid
    	if ($cate_id) {
    		$catids = D('Goodscate')->getChildren($cate_id);
    		if (!empty($catids)) {
    			$condition['cate_id'] = array('IN', $catids);
    		} else {
    	...
    	$condition['closed']=0;
    	$condition['audit']=1;
    	$condition['end_date']=array("egt",TODAY);
    	$condition['city_id']=$this->city_id;
    	$jp=D("Goods")->where($condition)->order("sold_num desc")->limit(0,6)->select();
    	$this->assign("jplist",$jp);
    	
    	
    	//$this->assign('cat',$catids);
    	//获取经度纬度
    	...
    	//所有产品
    	$list=D("Goods")->where($condition)->select();
    	foreach ($list as $k => $v){
    		$shopid=$v['shop_id'];
    		$map=array("shop_id"=>$shopid);
    		$shop=D("Shop")->where($map)->limit(0,1)->select();
    		foreach ($shop as $key => $val){
    			$list[$k]['d']=getDistance($lat, $lng, $val['lat'], $val['lng']);
    			$list[$k]['shopname']=$val['shop_name'];
    		}
    	}
    	
    	$this->assign("list",$list);
    	$this->assign('fenlei', $l);
    	$this->display();
...
//        dump($_COOKIE);die;
       ...
       foreach ($list as $k => $val){
       	...
       	// $lng1=$v['lng'];
       	// $lat1=$v['lat'];
       	 $list[$k]['d'] = getDistance($lat, $lng, $v['lat'], $v['lng']);
...
//        dump($list);die;
    ...
    public function lists($id){
    	$id=(int)$id;
    	$map=array("closed"=>0,"audit"=>1,"shopcate_id"=>$id);
    	$orderby=array("sold_num"=>"desc");
    	$list=D("Goods")->where($map)->order($orderby)->select();
    	$this->assign("list",$list);
...
//        dump($shop);die;
    ...
    public function combine()
    {
        ...
        foreach ($order as $k =>$val){
        	...
        	$condition=array("user_id"=>$val['user_id'],"shop_id"=>$val['shop_id'],"expire_date"=>array("EGT",TODAY),"is_used"=>0);
        	$coupon=D("Coupondownload")->where($condition)->order("money desc")->select();
        	
        	foreach ($coupon as $key =>$v){
        		
        		if($v['condition']<$val['need_pay']/100){
        			
        			$money+=$v['money'];
        			
        			//获取代金券id
        			$coupon_id[$i]=$v['download_id'];
        			$i+=1;
        			break;
        		}
        	}
