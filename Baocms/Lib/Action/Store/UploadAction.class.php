...
class  UploadAction extends  CommonAction{
    ...
    public function uploadify() {
        ...
        if (!$upload->upload()) {// 上传错误提示错误信息
            ...
            if($upload->thumb){
                if ($info[1]) {
                    $arr[] = $name . '/thumb_' . $info[0]['savename'];
                    $arr[] = $name . '/thumb_' . $info[1]['savename'];
                    return $arr;
                } else {
                    return $name . '/thumb_' . $info[0]['savename'];
                }
            }else{
                if ($info[1]) {
                    $arr[] = $name . '/' . $info[0]['savename'];
                    $arr[] = $name . '/' . $info[1]['savename'];
                    return $arr;
                } else {
                    return $name . '/' . $info[0]['savename'];
                }
            }
        }
    }
